		...
		var send = function(){
			...
			this.onload = function(){
				...
				for ( var i = 0; i < setcookies.length; i++ ) {
					// fix: cause 'isSSRReqCookie'-flag to be set to avoid "Can't set headers after they are sent."-exception
					// a page-load could make multiple ajax-requests (to the proxy) which set a cookie-value. the cookie-value could change with subsequent ajax-requests
					document.cookie = setcookies[ i ] + "; SSR-Mirror-In-DOM";
				}
				if(onload) {
					return onload.apply(this, arguments);
				}
